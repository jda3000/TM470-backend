...
class BeatsList(APIView):
    ...
    def get(self, request):
        ...
        if load_serializer.is_valid():
            ...
            if exclude_first:
                beats = Beat.objects.all().order_by('-date_created')[exclude_first:exclude_first + number_of_results]
            else:
                beats = Beat.objects.all().order_by('-date_created')[:number_of_results]
